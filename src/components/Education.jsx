      ...
      <div className="relative border-l-2 border-white/10 ml-3 space-y-8">
        {education.map((edu, index) => (
          <div key={index} className="relative pl-8">
            ...
            <div className="group bg-card-bg p-6 rounded-xl border border-white/5 hover:border-white/10 transition-colors">
              <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-4">
                 <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-white/5 rounded-lg p-2 flex items-center justify-center border border-white/10">
                      <img src={edu.logo} alt={edu.school} className="w-full h-full object-contain" />
                    </div>
                    <div>
                      <h3 className="text-lg font-bold text-white group-hover:text-gray-300 transition-colors">{edu.school}</h3>
                      <div className="text-sm text-gray-400 font-medium">{edu.degree}</div>
                    </div>
                 </div>
                 <div className="flex flex-col items-end gap-1 mt-2 sm:mt-0">
                    <span className="text-xs font-mono text-green-400">{edu.year}</span>
              ...
              <p className="text-gray-500 text-sm leading-relaxed mt-4 border-t border-white/5 pt-4">
                {edu.description}
              </p>
            </div>
          </div>
